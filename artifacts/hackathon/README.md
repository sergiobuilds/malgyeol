...
"섹션 5 수신자 보호 보완검증". 아래 값은 현재 입력 기준이며, 원문 JSON 이 갱신되면 같은 명령으로
문서·덱이 함께 다시 만들어진다.

- 확인됨: Node 198개 검사·타입 검사·의존성 검사 통과 기록, 설치 SDK 기반 Python 32개 검사 통과 기록, 실제 Node HTTP·SQLite와 Python 음성도구의 결합 검증 기록, 식사 경로 유지·생필품 재선택·시민 회신 기록의 로컬 통합 확인.
...
문서 본문의 검사 건수(Node 198건·Python 32건)는 현재 입력 기준이다. 실제 전화망
