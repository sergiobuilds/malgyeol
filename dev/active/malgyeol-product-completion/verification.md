...
아래 수치는 해커톤 수용검사의 실행 한도이며 서비스 운영 정책이나 실측 성능이 아님. 실제 번호·수신 경로 확보 후 적용.

| 항목 | 시험 기준 | 근거 및 기록 |
|---|---|---|
| 정상 왕복 | 독립 요청 2회 연속 성공 | 우연한 1회 성공과 구별, 두 번째 요청의 맥락 격리 확인 |
| 부분 해결·조건 변경·부재 | 각 1회, 결함 수정 후 해당 경로만 1회 재검사 | 불필요한 반복 발신 제한 |
| 발신 연결 대기 | 25초 | `coordination_voice.py`의 `agent.call(timeout=25)` |
| 통화 종료 대기 | 연결 후 150초 | 동일 파일 `call.wait()` 제한. 초과 시 완료로 집계 금지 |
| 첫 인사 | 연결 후 10초 이내 | 시연 수용 목표. 초과 시 해당 시나리오 실패 기록 |
| 음성 응답 | 발화 종료 후 10초 이내, 긴 침묵 15초 후 재질문 | 사람 청취 또는 양방향 오디오 실측 필요 |
| 기록·화면 반영 | 통화 종료 후 15초 이내 | worker 대기 3초·화면 조회 약3초에 전송 여유 반영. 오류는 숨기지 않음 |
| 전체 실행 | 최대 20통·60분, 먼저 도달한 한도에서 종료 | 검증 run별 누적 시도, 성공·실패·무응답 모두 산정 |
| 즉시 중단 | 허용 외 목적지, 역할 불일치, 개인정보 오공개, 동시 통화, 중복 신청 의사 전달 | 후속 자동 문의를 진행하지 않고 해당 원인 수정 후 재검사 |

시험 담당자는 각 run 시작·종료 UTC, 소스 커밋, SDK 버전, 비식별 요청·통화 참조, 발신/연결/첫 인사/종료/저장/화면 반영 시각을 비공개 기록으로 보존. 공개 결과에는 번호·토큰·통화 전문·원음을 넣지 않음. 자동 역할극과 사람이 청취한 결과를 구분.

이 기준은 시험 진행자의 중단 기준이며 모든 시간 한도가 코드에서 자동 강제된다는 주장이 아님. 수신자 없는 무인 실행에서 한도 감시·종료를 보장할 수 없다면 발신 시작 금지. 기관 역할의 부재는 준비된 B 수신 경로에서만 재현. 실제 기관 번호를 사용하거나 성공 응답을 주입하여 통과시키지 않음.
