...
export interface CoordinationHttpResult {
  status: number;
  body: unknown;
}
...
function bool(value: unknown): boolean {
  if (typeof value !== "boolean") throw new InvalidInput();
  return value;
}
...
export function createCoordinationRoutes(
  engine: CoordinationEngine,
  credentials: { operator?: string; agent?: string },
) {
  return (
    method: string,
    url: URL,
    ...
    if (
      ...
      !url.pathname.startsWith("/api/coordination/")
    )
      return undefined;
    const reply = (status: number, value: unknown): CoordinationHttpResult => ({
      status,
      body: value,
    });
    ...
    try {
      if (url.pathname.startsWith("/api/support/")) {
        ...
        if (url.pathname === "/api/support/programs")
          return reply(200, { programs: listPrograms() });
        if (url.pathname === "/api/support/institutions") {
          ...
          return reply(200, {
            institutions: listInstitutions({
              ...(programId ? { programId: choice(programId, programs) } : {}),
              ...(district ? { district } : {}),
              ...(query ? { query } : {}),
            }),
          });
        }
        const institutionId = url.pathname.match(
          /^\/api\/support\/institutions\/([^/]+)$/,
        )?.[1];
        const institution = institutionId
          ? getInstitution(institutionId)
          : undefined;
        return institution
          ? reply(200, { institution })
      ...
      if (
        ...
        return error(403, "FORBIDDEN", "담당자 연결이 필요합니다.");
      if (url.pathname === "/api/coordination/requests") {
        if (method === "GET") {
          const refs = url.searchParams.getAll("citizenRef");
          if (refs.length > 1) throw new InvalidInput();
          if (refs.length) str(refs[0], 160);
          return reply(200, { requests: engine.listRequests(refs[0]) });
        }
        if (method === "POST") {
          if (
            !Array.isArray(body.needs) ||
            body.needs.length < 1 ||
            body.needs.length > 20
          )
            throw new InvalidInput();
          return reply(201, {
            request: engine.createRequest({
              ...
              needs: body.needs.map((v) => {
                const n = record(v);
                return {
                  description: str(n.description),
                  category: str(n.category, 100),
                  ...(n.constraints === undefined
                    ? {}
                    : { constraints: strings(n.constraints) }),
                  ...(n.requestDetails === undefined
                    ? {}
                    : {
                        requestDetails: validateRequestDetails(
                          n.requestDetails,
                        ),
                      }),
                };
      ...
      const id = m[1]!,
        action = m[2] ?? "";
      if (!engine.getRequest(id))
        return error(404, "NOT_FOUND", "요청을 찾을 수 없습니다.");
      if (method === "GET" && !action)
        return reply(200, { request: engine.getRequest(id) });
      if (method === "GET" && action === "events")
        return reply(200, { events: engine.events(id) });
      ...
      if (action === "consent")
        return reply(200, {
          request: engine.recordConsent(id, {
            purpose: str(body.purpose),
            institutionIds: strings(body.institutionIds),
            sharedFields: strings(body.sharedFields),
            allowCoordination: bool(body.allowCoordination),
          }),
        });
      if (action === "inquiries") {
        const institutionId = str(body.institutionId, 160),
          programId = choice(body.programId, programs),
          contactPurpose = str(body.contactPurpose);
        const institution = getInstitution(institutionId);
        if (
          !institution?.programs.some((p) => p.programId === programId) ||
          !institution.contacts.some((c) => c.purpose === contactPurpose)
        )
          return error(
            400,
            "INVALID_CONTACT",
            "해당 사업의 문의 창구를 선택해 주세요.",
          );
        return reply(201, {
          inquiry: engine.addInquiry(id, {
            needId: str(body.needId, 160),
            institutionId,
            programId,
            contactPurpose,
      ...
      if (attempt)
        return reply(200, {
          attempt: engine.startAttempt(
            id,
            attempt[1]!,
            str(body.idempotencyKey, 160),
          ),
        });
      const retry = action.match(/^inquiries\/([^/]+)\/retry$/);
      if (retry)
        return reply(200, { request: engine.retryInquiry(id, retry[1]!) });
      const result = action.match(/^attempts\/([^/]+)\/result$/);
      if (result)
        return reply(200, {
          request: engine.finishAttempt(id, result[1]!, {
            status: choice<AttemptResult["status"]>(body.status, [
              "completed",
              "no-answer",
              "failed",
              "unknown",
            ]),
            ...(body.providerCallId === undefined
              ? {}
              : { providerCallId: str(body.providerCallId, 160) }),
          }),
        });
      const answer = action.match(/^inquiries\/([^/]+)\/answer$/);
      if (answer)
        return reply(200, {
          request: engine.recordAnswer(id, answer[1]!, {
            outcome: choice<AnswerInput["outcome"]>(body.outcome, [
              "available",
              "declined",
              "alternative",
            ]),
            ...
            requiresChoice: bool(body.requiresChoice),
          }),
        });
      ...
      if (need)
        return reply(200, {
          request:
            need[2] === "stop"
              ? engine.stopNeed(id, need[1]!)
              : engine.recordChoice(id, need[1]!, str(body.choice)),
        });
      if (action === "callback")
        return reply(200, {
      ...
      if (e instanceof InvalidInput)
        return error(400, "INVALID_INPUT", "입력 내용을 확인해 주세요.");
      if (e instanceof CoordinationError)
        return error(
          e.status,
          e.code,
          "현재 진행 상태와 요청 내용을 확인해 주세요.",
        );
      throw e;
    }
  };
